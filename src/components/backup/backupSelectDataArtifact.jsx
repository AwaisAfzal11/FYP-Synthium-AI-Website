...
//       {/* 2 main buttons: i) Model ii) Setting */}
...
//                     Choose File
//                   </button>
//                   <p className="text-[15px] mt-[2px] mb-[30px]">or simply drag your desired file and drop here</p>
...
//           {/* Button 2: connect from already uploaded files*/}
//           <div className="mb-4 w-full">
//             <div
//               className="flex items-center justify-between cursor-pointer p-4 bg-black rounded"
...
//           <button onClick={handleSelectDataArtifact} className="bg-color-7 py-[10px] px-[15px] text-white text-[20px] mb-[15px] mt-[3px] border-white rounded-md">
//             Continue
//           </button>
//         </div>
//       </div>
...
const FileUploadComponent = ({ onFileUpload }) => {
  ...
  return (
    <div className="flex flex-col items-center">
      <div
        {...getRootProps()}
        className={`border-2 border-dashed p-8 rounded-lg mb-4 w-full  text-center cursor-pointer ${isDragActive ? 'border-purple-500 bg-purple-100' : 'border-gray-300'
          }`}
      >
        <input {...getInputProps()} />
        {isDragActive ? (
          ...
          <p className=''>Click to select a file, Drag and drop a CSV file here</p>
        )}
      </div>
      {file && (
        <p className="mb-4">
      ...
      <button
        ...
        className={`bg-color-7 py-[10px] px-[15px] text-white text-[20px] mb-[15px] mt-[3px] border-white rounded-md ${!file || isUploading ? 'opacity-50 cursor-not-allowed' : ''
          }`}
      >
        {isUploading ? (
          <>
...
function SelectDataArtifact({ onFormSubmit, projectData }) {
  ...
  useEffect(() => {
    ...
    get_all_data_artifacts(accessToken)
      .then((response) => {
        ...
        console.log(response);
        setDataArtifacts(response.data.data_artifacts);
        set_get_data_artifact_Loading(false);
      })
      .catch((error) => {
        console.error(error);
        set_get_data_artifact_Loading(false);
  ...
  const handleSelectDataArtifact = () => {
    if (selectedDataArtifactId != null) {
      console.log(selectedDataArtifactId);
      onFormSubmit(selectedDataArtifactId);
    }
  };
  ...
  const handleFileUpload = async (file) => {
    const accessToken = sessionStorage.getItem('access_token');
    try {
      const response = await uploadDataArtifact(accessToken, file);
      console.log("Data Artifacts Uploaded Successfully...");
      console.log(response);
      setSelectedDataArtifactId(response.data.data_artifact_id);
      onFormSubmit(response.data.data_artifact_id);
              ...
              {openSection === 2 && (
                <div className="mt-2 p-4 border border-gray-700 rounded bg-black">
                  <form className="ml-[30px] mb-[30px] max-w-sm">
                    ...
                    <select
                      id="data_artifact"
                      className="bg-gray-50 border border-color-7 text-gray-900 text-sm rounded-lg focus:ring-purple-500 focus:border-color-7 block w-full p-2.5 dark:bg-black dark:border-color-7 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500 custom-select"
                      onChange={handleDataArtifactSelectionChange}
                    >
                      <option key={"Default Value"}>Choose from your previous data artifacts.</option>
                      {dataArtifacts.map((dataArtifact) => (
                  ...
                  <div className="flex justify-center items-center ">
                    <button
                      onClick={handleSelectDataArtifact}
                      className="bg-color-7 py-[10px] px-[15px] text-white text-[20px] mb-[15px] mt-[3px] border-white rounded-md"
                      disabled={!selectedDataArtifactId}
                    >
                      Continue
                    </button>
                  </div>
